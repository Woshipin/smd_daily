...
namespace App\Filament\Resources\WinboxThFootball1Resource\Widgets;
...
class WinboxThFootballInformation1ResourceWidget extends BaseWidget
{
    // 设置表格标题
    protected int | string | array $columnSpan = 'full'; // 让表格占满全宽
    protected static ?string $heading = '足球赛事资讯列表 - 1 (th) - Information';

    public function table(Table $table): Table
    {
        return $table
            ->query(
                // 这里指定使用 WinboxThFootballInformation1 模型
                WinboxThFootballInformation1::query()
            )
            ->columns([
